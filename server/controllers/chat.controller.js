...
// Create a new group chat   ===>  /chat/new
...
// add members in group  ==> /chat/addmembers
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString())
    next(new ErrorHandler("You are not allowed to add memebers", 403));
  ...
  return res.status(200).json({
    success: true,
    message: "Members Addded Successfully",
  });
});

// remove members in group  ==> /chat/removemembers
...
const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  if (files.length < 1) return next(new ErrorHandler("Please add files", 400));
...
// get chat/group details  ==> /chat/:id
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (!chat.groupChat && !chat.members.includes(req.user.toString())) {
    return next(new ErrorHandler("Only group members can delete group", 403));
  }
